...
#[serde(bound = "P: Fp384Parameters")]
pub struct Fp384<P: Fp384Parameters>(pub(crate) BigInteger, #[serde(skip)] PhantomData<P>);
...
impl<P: Fp384Parameters> UniformRandom for Fp384<P> {
    /// Samples a uniformly random field element.
    #[inline]
    fn uniform<R: RngCore + CryptoRng>(r: &mut R) -> Self {
        loop {
            let mut tmp = Fp384::<P>(BigInteger::uniform(r), PhantomData);
...
impl_ops_traits!(Fp384, Fp384Parameters);
