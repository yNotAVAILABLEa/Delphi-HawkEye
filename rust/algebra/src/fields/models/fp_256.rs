...
#[serde(bound = "P: Fp256Parameters")]
pub struct Fp256<P: Fp256Parameters>(pub(crate) BigInteger, #[serde(skip)] PhantomData<P>);
...
impl<P: Fp256Parameters> UniformRandom for Fp256<P> {
    /// Samples a uniformly random field element.
    #[inline]
    fn uniform<R: RngCore + CryptoRng>(r: &mut R) -> Self {
        loop {
            let mut tmp = Fp256::<P>(BigInteger::uniform(r), PhantomData);
...
impl_ops_traits!(Fp256, Fp256Parameters);
