...
use std::{
    fmt::Debug, io::{Read, Write}, marker::PhantomData
};
    ...
    P::Field: PrimeField<BigInt = <<P::Field as PrimeField>::Params as FpParameters>::BigInt>,
{
    pub fn offline_server_protocol<RNG: CryptoRng + RngCore>(
        ...
        // println!("pos1 reader communication size:{:?}", reader.count());
        let mut num_relu = 0;
        ...
        for (i, layer) in neural_network.layers.iter().enumerate() {
            match layer {
                ...
                Layer::LL(layer) => {
                    println!("start linear reader communication size:{:?}", reader.count());
                    println!("start linear writer communication size:{:?}", writer.count());
                    println!("linear Layer input dim: {:?}", layer.input_dimensions());
                    let startreader = reader.count();
                    let startwriter = writer.count();
                    let randomizer = match &layer {
                        LinearLayer::Conv2d { .. } | LinearLayer::FullyConnected { .. } => {
                            ...
                            // println!("pos3 reader communication size:{:?}", reader.count());
                    ...
                    println!("end linear reader communication size:{:?}", reader.count());
        ...
        println!("start relu writer communication size:{:?}", writer.count());
        let startreader = reader.count();
        let startwriter = writer.count();
        let relu_time =
        ...
        timer_end!(relu_time);
        println!("end relu reader communication size:{:?}", reader.count());
        ...
        for (i, layer) in neural_network_architecture.layers.iter().enumerate() {
            match layer {
                ...
                LayerInfo::LL(dims, linear_layer_info) => {
                    ...
                    // `F` reduces at the end of each layer.
        ...
        for (i, layer) in neural_network.layers.iter().enumerate() {
            match layer {
                Layer::NLL(NonLinearLayer::ReLU(dims)) => {
                    ...
                    println!("start relu reader communication size:{:?}", reader.count());
                    println!("start relu writer communication size:{:?}", writer.count());
                    println!("relu Layer dim: {:?}", layer.input_dimensions());
                    let startreader = reader.count();
                    let startwriter = writer.count();
                ...
                Layer::NLL(NonLinearLayer::PolyApprox { dims, poly, .. }) => {
                    ...
                    println!("start approx communication size{:?}", reader.count());
                    let layer_size = next_layer_input.len();
                    assert_eq!(dims.input_dimensions(), next_layer_input.dim());
                    ...
                    next_layer_derandomizer = ndarray::Array1::from_iter(shares_of_eval)
                        ...
                        .into();
                    timer_end!(start_time);
                    println!("end approx communication size{:?}", reader.count());
                }
                Layer::LL(layer) => {
        ...
        for (i, layer) in architecture.layers.iter().enumerate() {
            match layer {
                ...
                LayerInfo::LL(_, layer_info) => {
                    ...
                    println!("linear writer communication size pos1:{:?}", writer.count());
                    println!("current data dim:{:?}", input.dim());
                    ...
                    println!("linear writer communication size pos6:{:?}", writer.count());
