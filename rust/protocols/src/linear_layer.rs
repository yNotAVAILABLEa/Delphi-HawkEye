    ...
    pub fn online_client_protocol(
        ...
        match layer {
            LinearLayerInfo::Conv2d { .. } | LinearLayerInfo::FullyConnected => {
                // println!("linear writer communication size pos2:{:?}", writer.count());
                let sent_message = MsgSend::new(x_s);
                // println!("Size of x_s: {:?} bytes", mem::size_of_val(x_s));
                // println!("sent_message{:?}", sent_message.)
                crate::bytes::serialize(writer, &sent_message)?;
                // println!("linear writer communication size pos3:{:?}", writer.count());
            }
            _ => {
                // println!("linear writer communication size pos4:{:?}", writer.count());
                layer.evaluate_naive(x_s, next_layer_input);
                for elem in next_layer_input.iter_mut() {
                    elem.inner.signed_reduce_in_place();
                }
                // println!("linear writer communication size pos5:{:?}", writer.count());
            }
        }
        timer_end!(start);
        Ok(())
