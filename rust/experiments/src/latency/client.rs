...
pub fn nn_client<R: RngCore + CryptoRng>(
    ...
    let tmp = generate_random_number(rng).1;
    let tmp2 = generate_random_number(rng);
    println!("Size of tmp: {:?} bytes", mem::size_of_val(&tmp));
    println!("Size of tmp2: {:?} bytes", mem::size_of_val(&tmp2));
